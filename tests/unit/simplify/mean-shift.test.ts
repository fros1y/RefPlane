...
describe('meanShiftFilter', () => {
  it('preserves a uniform image', async () => {
    ...
    const result = await meanShiftFilter(image, 10, 20);
  ...
  it('produces output with same dimensions as input', async () => {
    ...
    const result = await meanShiftFilter(image, 5, 10);
  ...
  it('converges similar nearby colors', async () => {
    ...
    const result = await meanShiftFilter(image, 8, 20);
  ...
  it('accepts a progress callback', async () => {
    ...
    await meanShiftFilter(image, 5, 10, (percent) => { updates.push(percent); });
