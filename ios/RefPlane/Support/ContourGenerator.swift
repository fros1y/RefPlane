...
enum ContourGenerator {
    ...
    static func generateSegments(
        ...
        segments.reserveCapacity(levels * gridW * 2)
        ...
        for row in 0..<gridH {
            for col in 0..<gridW {
                let tl = vertices[row * vertW + col]
                let tr = vertices[row * vertW + col + 1]
                let bl = vertices[(row + 1) * vertW + col]
                let br = vertices[(row + 1) * vertW + col + 1]

                // Skip cells entirely in the background zone
                if tl >= backgroundCutoff && tr >= backgroundCutoff &&
                   bl >= backgroundCutoff && br >= backgroundCutoff {
                    continue
                }
