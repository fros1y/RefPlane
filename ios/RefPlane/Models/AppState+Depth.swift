...
extension AppState {
    ...
    func computeDepthMap() {
        ...
        pipeline.processingLabel = "Estimating depth…"
    ...
    func applyDepthEffects() {
        ...
        guard depth.depthConfig.enabled, let depthMapImage = depth.depthMap else {
            depth.depthProcessedImage = nil
            return
        }
        ...
        pipeline.isProcessing = true
        pipeline.processingLabel = "Applying depth…"
        pipeline.processingIsIndeterminate = true
        ...
        depthEffectTask = Task { @MainActor [weak self] in
            ...
            let result = await Task.detached(priority: .userInitiated) {
                self.depthEffectOperation(sourceImage, depthMapImage, config)
            }.value
    ...
    func updateDepthThresholdPreview() {
        guard let depthMapImage = depth.depthMap, let source = depthEffectSourceImage() else {
            depth.isEditingDepthThreshold = false
            depth.depthThresholdPreview = nil
            depth.cachedDepthTexture = nil
            depth.cachedSourceTexture = nil
        ...
        if let coverage = backgroundCoverage(in: depthMapImage, cutoff: bg) {
    ...
    func logDepthDiagnostics(event: String, depth depthImage: UIImage) {
        let range = depth.depthRange
        let sourceSize = displayBaseImage?.size ?? .zero
        let depthSize = depthImage.size
        let coverage = backgroundCoverage(in: depthImage, cutoff: depth.depthConfig.backgroundCutoff)
        let pixels = grayscalePixels(from: depthImage)
        ...
        let foregroundCutoff = depth.depthConfig.foregroundCutoff
        let backgroundCutoff = depth.depthConfig.backgroundCutoff
    ...
    func recomputeContours() {
        contourTask?.cancel()
        guard transform.contourConfig.enabled, let depthMapImage = depth.depthMap else {
            depth.contourSegments = []
            return
        }
