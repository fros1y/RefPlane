...
extension AppState {
    ...
    var shouldShowPreviousSettingsOption: Bool {
        guard let previousTransformSnapshot = transform.previousTransformSnapshot else { return true }
    ...
    func selectTransformPreset(_ selection: TransformPresetSelection) {
        ...
        case .saved(let presetID):
            guard let preset = transform.savedTransformPresets.first(where: { $0.id == presetID }) else { return }
    ...
    func restoreInitialTransformSnapshotSelection() {
        guard let prev = transform.presetManager.previousSnapshot else {
